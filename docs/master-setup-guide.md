...
| **Protocols Detection** | `PROTOCOLS_DETECTION_` | **Google Gemini** reads the Markdown, seeded with the author's protocol rows ("Section 0"); reconciled in `krt_grounding` instead. A post-filter reclassifies purely computational "protocols" as software. | `prompts/{seeded,blind}/protocols-detection.txt` |
