...
4. **Merge rows**: select two or more rows, click "Merge…", and choose which value to keep per column
5. **Quick actions on empty cells**: an empty Identifier cell offers "No identifier exists" and
   "Identifier pending" in one click; an empty Source cell offers "None". These are the exact phrases
   the validator accepts when no real identifier exists
6. **Batch fixes**: when several rows share the same fixable issue, apply the fix to all at once from
   the Quick Fixes panel above the table

**Finding your way around a large KRT:**

- **Filter tabs** — one per resource type, each showing a count; a tab with errors is marked
- **Row order** — switch between "As submitted" (your original order) and "By resource type"
- **Search** — free-text filter across rows
- **Column sorting** — click any column header
- **Resizable columns** — drag a header edge; your widths are remembered
- **Jump to problems** — click the error, warning or suggestion counters to go straight to the first one
- **Download** — export the current table at any time

**One thing the app does for you automatically:** if you left the Identifier cell empty but pasted a
real identifier (an RRID, DOI, or sequence) into Additional Information, the app moves it into the
Identifier column for you and saves it. Nothing else is ever changed without you accepting it.

All changes are automatically saved and tracked with full history.

> For a complete list of everything the table can do, see the
> [KRT Editor reference](./krt-editor.md).
...
2. The Job Status panel shows every detection (DAS extraction, software, ORCID, materials, markdown conversion, datasets, protocols, identifier matching), the PDF Analysis step that builds the Generated KRT, and the AI Suggestions step that compares it against your KRT
...
- A short **reason** for the suggestion and the detection module(s) it came from

**Badges you may see on a suggestion:**

- **In KRT** — this resource is already in your KRT, so there's nothing to add (hover to see which row it matched)
- **Update** — it matches a row you already have, so accepting will update that row instead of adding a new one
- **Verify** — the analysis found no identifier for this one, so please check it before adding

**You can edit a suggestion before accepting it.** The proposed values in a suggested row are
editable in place — correct anything that's wrong, then accept. Only the fields you actually changed
are recorded as your edits.

**Rejecting:** you can add an optional reason when you reject a suggestion. This is helpful for the
curation team.

**Handling many suggestions at once:** tick the checkboxes (or "select all visible") and use the bulk
bar:

| Button | What it does |
|---|---|
| Approve selected | Accepts them all — asks you to confirm at 5 or more |
| Approve with Resource Type… | Accepts them all while setting the resource type in one go |
| Reject selected | Rejects them all |
| Edit column… | Sets one column to the same value across every selected row |
| Merge… | Combines 2+ selected rows into one |
| Delete selected | Deletes the selected rows |

⚠️ **You must accept or reject every suggestion before you can continue to Step 3.** The Continue
button will tell you how many are left.

Suggestions are generated by an AI comparison of your KRT against what the analysis found, and they are **saved** — they don't silently change as you edit your KRT. If you make changes and want a fresh set, use the **Regenerate suggestions** button. Accepting a "remove" suggestion deletes that row from your KRT.
...
> This screen is **review-only** — it's a different table from the editor you used in Steps 1 and 2,
> so there's no editing, searching or bulk actions here. Use the "Show changes" toggle and click the
> **AI / Auto-fix / User** legend entries to hide or show changes from each source. Row colours mean
> added / updated / deleted; an edited cell is marked with a dotted underline and a `?`.
...
3. You land on **step 2 (KRT)** for round N+1. The page shows your KRT (existing or blank) with the pipeline already running on the new PDF.
