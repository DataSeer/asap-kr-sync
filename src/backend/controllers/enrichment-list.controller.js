...
async function importEntries(req, res) {
  ...
  if (mode === 'replace' && defaultType) {
    ...
    deletedCount = await EnrichmentListEntry.destroy({ where: { category, resourceType: defaultType } });
  } else if (mode === 'replace') {
    // Full replace: delete all entries for this category
    deletedCount = await EnrichmentListEntry.destroy({ where: { category } });
  }

  const created = await EnrichmentListEntry.bulkCreate(records);
