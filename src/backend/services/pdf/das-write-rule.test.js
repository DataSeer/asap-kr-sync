/**
 * Whose Availability Statement wins when extraction runs again.
 *
 * Two fields, two meanings — and since the apply split, two different KINDS of
 * thing:
 *
 *   - `extractedDataAvailabilityStatement` is what the last extraction found,
 *     always overwritten. A projection of the newest execution's output.
 *   - `dataAvailabilityStatement` is what the submission STANDS ON. Extraction
 *     no longer writes it; it offers the value, and an APPLY promotes it — only
 *     while the field is empty, attributed to the execution that produced it,
 *     and recorded on `change_logs`.
 *
 * The bug this pins: extraction wrote the second field every time,
 * unconditionally. An author whose statement the extractor could not find typed
 * one by hand — the whole reason the manual path exists — and the next run of
 * extraction replaced it with "Not found". The app undid their work and called
 * it an update, silently, with no record that anything had been lost.
 *
 * The rule itself now lives in apply.service, so these tests exercise it
 * through the real service and assert on what it records — silence is half of
 * what went wrong the first time.
 *
 * Run with: node --test src/backend/services/pdf/das-write-rule.test.js
 */

const { test } = require('node:test');
const assert = require('node:assert/strict');

const { applyExtractedDas } = require('./pdf.service');
const { NO_DAS_SENTINEL } = require('../das-suggestions/das-suggestions.service');
const models = require('../../models');

const CONFIRMED = new Date('2026-08-20T09:00:00Z');

/**
 * Capture the change-log rows an apply writes, instead of a database.
 *
 * Returns the array; each test that cares reads it directly. Tests that do not
 * still need it, because an unmocked ChangeLog.create reaches for a connection.
 */
function captureApplies(t) {
  const written = [];
  t.mock.method(models.ChangeLog, 'create', async (row) => {
    written.push(row);
    return { id: `log-${written.length}`, ...row };
  });
  return written;
}

/** A submission as it exists after a previous extraction the author left alone. */
function untouched(extraction = 'Data are at Zenodo.') {
  return {
    id: 'sub-1',
    currentRound: 1,
    extractedDataAvailabilityStatement: extraction,
    dataAvailabilityStatement: extraction,
    dasConfirmedAt: CONFIRMED,
    dasConfirmedByUserId: 'user-1',
    async save() { return this; }
  };
}

test('a fresh extraction fills an empty submission', async (t) => {
  captureApplies(t);
  const submission = {
    id: 'sub-1',
    currentRound: 1,
    extractedDataAvailabilityStatement: null,
    dataAvailabilityStatement: null,
    dasConfirmedAt: null,
    dasConfirmedByUserId: null,
    async save() { return this; }
  };

  const outcome = await applyExtractedDas(submission, 'Data are at Zenodo.');

  assert.equal(submission.dataAvailabilityStatement, 'Data are at Zenodo.');
  assert.equal(submission.extractedDataAvailabilityStatement, 'Data are at Zenodo.');
  assert.equal(outcome.replaced, true);
});

test('re-extraction does not overwrite a field that is already filled', async (t) => {
  captureApplies(t);
  // Even though nobody edited it: once the working field holds a statement, it
  // belongs to whoever put it there, and the app cannot tell a statement the
  // author read and accepted from one they never looked at.
  //
  // The newer reading is not lost — it goes to the extracted field, and the
  // page shows the two side by side when they differ.
  const submission = untouched();

  const outcome = await applyExtractedDas(submission, 'Data are at Dryad, accession 12345.');

  assert.equal(submission.dataAvailabilityStatement, 'Data are at Zenodo.');
  assert.equal(submission.extractedDataAvailabilityStatement, 'Data are at Dryad, accession 12345.');
  assert.equal(outcome.replaced, false);
});

test('"Not found" does not count as filled', async (t) => {
  captureApplies(t);
  // Extraction is fail-soft and always persists something, so a first pass that
  // found nothing leaves the sentinel in the working field. Treating that as
  // occupied would lock out every later extraction — including the one that
  // finally succeeds after the manuscript is re-uploaded.
  const submission = untouched(NO_DAS_SENTINEL);

  await applyExtractedDas(submission, 'Data are at Zenodo.');

  assert.equal(submission.dataAvailabilityStatement, 'Data are at Zenodo.');
});

test('re-extraction does NOT overwrite what the author typed', async (t) => {
  captureApplies(t);
  const submission = untouched(NO_DAS_SENTINEL);
  submission.dataAvailabilityStatement = 'All data are in the supplement.';

  const outcome = await applyExtractedDas(submission, NO_DAS_SENTINEL);

  assert.equal(submission.dataAvailabilityStatement, 'All data are in the supplement.',
    'the author supplied this precisely because extraction could not');
  assert.equal(outcome.replaced, false);
});

test('and it does not overwrite an author correction with a better guess either', async (t) => {
  captureApplies(t);
  // The case the two fields exist for: extraction has something to say this
  // time, and it is still not the author's text to overwrite.
  const submission = untouched('Data are at Zenodo.');
  submission.dataAvailabilityStatement = 'Data are at Zenodo, DOI 10.5281/zenodo.1.';

  await applyExtractedDas(submission, 'Data available on request.');

  assert.equal(submission.dataAvailabilityStatement, 'Data are at Zenodo, DOI 10.5281/zenodo.1.');
  assert.equal(submission.extractedDataAvailabilityStatement, 'Data available on request.',
    'the new reading is still recorded — it is just not promoted over a person');
});

test('a filled statement keeps its confirmation', async (t) => {
  captureApplies(t);
  // Extraction running again changed nothing the author agreed to, so making
  // them agree again would be noise — and would park the Availability check
  // awaiting input for a reason no user could see.
  const submission = untouched(NO_DAS_SENTINEL);
  submission.dataAvailabilityStatement = 'All data are in the supplement.';

  const outcome = await applyExtractedDas(submission, NO_DAS_SENTINEL);

  assert.equal(submission.dasConfirmedAt, CONFIRMED);
  assert.equal(outcome.confirmationWithdrawn, false);
});

test('filling an empty field withdraws any confirmation standing over it', async (t) => {
  captureApplies(t);
  // Extractor-authored text has nobody behind it. A confirmation left from
  // earlier words does not carry over — the check would report on a statement
  // nobody has read, in the author's name.
  const submission = untouched(NO_DAS_SENTINEL);

  const outcome = await applyExtractedDas(submission, 'Data available on request.');

  assert.equal(submission.dasConfirmedAt, null);
  assert.equal(submission.dasConfirmedByUserId, null);
  assert.equal(outcome.confirmationWithdrawn, true);
});

test('a blank field is filled, whatever shape the blank is', async (t) => {
  captureApplies(t);
  // '', null and undefined all mean "nothing there". Any of them freezing
  // extraction out would leave the submission with no statement at all.
  for (const blank of ['', '   ', null, undefined]) {
    const submission = {
      id: 'sub-1',
      currentRound: 1,
      extractedDataAvailabilityStatement: 'Data are at Zenodo.',
      dataAvailabilityStatement: blank,
      dasConfirmedAt: null,
      dasConfirmedByUserId: null,
      async save() { return this; }
    };

    await applyExtractedDas(submission, 'Data are at Dryad.');

    assert.equal(submission.dataAvailabilityStatement, 'Data are at Dryad.',
      `${JSON.stringify(blank)} must not lock extraction out`);
  }
});

test('the extracted field is always updated, filled or not', async (t) => {
  captureApplies(t);
  // It is the record of what the extractor said on this run. A reader comparing
  // the two fields is asking "did the manuscript change?", and a stale
  // extracted value answers a question about some earlier run instead.
  const filled = untouched();
  await applyExtractedDas(filled, 'Data available on request.');
  assert.equal(filled.extractedDataAvailabilityStatement, 'Data available on request.');

  const empty = {
    id: 'sub-1', currentRound: 1,
    extractedDataAvailabilityStatement: 'old', dataAvailabilityStatement: '',
    dasConfirmedAt: null, async save() { return this; }
  };
  await applyExtractedDas(empty, 'Data available on request.');
  assert.equal(empty.extractedDataAvailabilityStatement, 'Data available on request.');
});

// ─────────────────────────────────────────────────────────────────────────────
// The apply is recorded
//
// Silence is half of what went wrong the first time: the write happened, the
// author's text vanished, and nothing anywhere said so. A value promoted into
// the submission now leaves a row saying which execution produced it and that
// nobody chose it.
// ─────────────────────────────────────────────────────────────────────────────

test('promoting a statement records where it came from', async (t) => {
  const applies = captureApplies(t);
  const submission = {
    id: 'sub-1', currentRound: 2,
    extractedDataAvailabilityStatement: null, dataAvailabilityStatement: null,
    dasConfirmedAt: null, dasConfirmedByUserId: null,
    async save() { return this; }
  };

  await applyExtractedDas(submission, 'Data are at Zenodo.', { stepExecutionId: 'exec-7' });

  assert.equal(applies.length, 1);
  const row = applies[0];
  assert.equal(row.action, 'apply');
  // Nobody chose this, and the log says so rather than crediting the owner.
  assert.equal(row.source, 'pipeline');
  assert.equal(row.userId, null);
  assert.equal(row.stepExecutionId, 'exec-7');
  assert.equal(row.columnName, 'data_availability_statement');
  assert.equal(row.newValue, 'Data are at Zenodo.');
  assert.equal(row.round, 2);
});

test('a refused promotion records nothing', async (t) => {
  const applies = captureApplies(t);
  const submission = untouched('Data are at Zenodo.');
  submission.dataAvailabilityStatement = 'All data are in the supplement.';

  await applyExtractedDas(submission, 'Data available on request.', { stepExecutionId: 'exec-8' });

  // The extraction still happened and is still recorded — as an execution, not
  // as a change to the submission, because nothing about the submission changed.
  assert.deepEqual(applies, []);
  assert.equal(submission.extractedDataAvailabilityStatement, 'Data available on request.');
});

test('an extraction with no execution behind it still applies', async (t) => {
  // A step run outside the pipeline, or one whose history write failed. The
  // apply is recorded without a source, which is worse than attributed and far
  // better than refusing to write a value the user is waiting for.
  const applies = captureApplies(t);
  const submission = {
    id: 'sub-1', currentRound: 1,
    extractedDataAvailabilityStatement: null, dataAvailabilityStatement: null,
    dasConfirmedAt: null, dasConfirmedByUserId: null,
    async save() { return this; }
  };

  await applyExtractedDas(submission, 'Data are at Zenodo.');

  assert.equal(submission.dataAvailabilityStatement, 'Data are at Zenodo.');
  assert.equal(applies[0].stepExecutionId, null);
});

test('a change log that will not write does not undo a correct value', async (t) => {
  // The value is on the submission by the time the log is attempted. Rolling it
  // back to protect the audit trail would trade working software for
  // bookkeeping — the failure is logged loudly instead.
  t.mock.method(models.ChangeLog, 'create', async () => { throw new Error('db is on fire'); });
  const submission = {
    id: 'sub-1', currentRound: 1,
    extractedDataAvailabilityStatement: null, dataAvailabilityStatement: null,
    dasConfirmedAt: null, dasConfirmedByUserId: null,
    async save() { return this; }
  };

  const outcome = await applyExtractedDas(submission, 'Data are at Zenodo.');

  assert.equal(submission.dataAvailabilityStatement, 'Data are at Zenodo.');
  assert.equal(outcome.replaced, true);
});

test('the projection is saved even when nothing is promoted', async (t) => {
  // `applyToSubmission` saves only when it writes, so a rejected reading would
  // otherwise be computed and thrown away — and the page shows it beside the
  // statement precisely so the two can be compared.
  captureApplies(t);
  let saves = 0;
  const submission = untouched('Data are at Zenodo.');
  submission.save = async function() { saves += 1; return this; };

  await applyExtractedDas(submission, 'Data are at Dryad.');

  assert.equal(saves, 1);
  assert.equal(submission.extractedDataAvailabilityStatement, 'Data are at Dryad.');
  assert.equal(submission.dataAvailabilityStatement, 'Data are at Zenodo.');
});
