...
async function detectSoftwareForSubmission(submission, jobLogger) {
  ...
  return {
    ...
    meta: {
      ...
      // `enabled` means CONFIGURED, not ran: the skip paths return it true
      // with a reason beside it. Both have to hold, or a run where the
      // markdown was missing still names a model nothing called.
      model: lm.enabled && !lm.skippedReason
        ? require('../../config/software-detection-lm-api').model
        : null,
      lmMs: lm.durationMs,
